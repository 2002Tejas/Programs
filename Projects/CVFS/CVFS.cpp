...
typedef struct superblock  // yat 2 members ahet 
{
    int TotalInodes;
    int FreeInode;
}SUPERBLOCK,*PSUPERBLOCK;

typedef struct inode     //yat 10 members ahet 
{
    char FileName[50];
    int InodeNumber;
    int FileSize;
...
typedef struct fileteble  // yat 5 members ahet 
{
    int readoffset;
    int writeoffset;
    int count;
...
void man(char *name)
{
    ...
    {
        printf("Description : Used to list all the data\n");
        printf("Usage : ls\n");
    }
    ...
    {
        printf("Descriotor : used to remove data from file\n");
    ...
    {
        printf("Descriptor : Used to close open file\n");
    ...
    {
        printf("Descrription : used to change file offset\n");
        printf("Usage : lseek File_Name ChangeInOffset StatPoint\n");
    }
...
void DisplayHelp()
{
    ...
    printf("stat : To Diaplay information of file using name\n");
    printf("fstat :To Display information of file using file descriptor\n");
    printf("truncate : To Remove all file data from\n");
...
int CloseByFileName(char *name)
{
    ...
    UFDTArr[i].ptrfiletable->readoffset = 0;  // ya aivaji aapan CloseFileByName(int fd) la call karu shakato
    UFDTArr[i].ptrfiletable->writeoffset = 0;
    (UFDTArr[i].ptrfiletable->ptrinode->ReferenceCount)--;
...
void ls_file()
{
    ...
    printf("\nFile Name\tInode numbers\tFileSize\tLink count\n");
    printf("----------------------------------------------------------\n");
    while(temp != NULL)
    {
        if(temp->FileType != 0)
...
int main()
{
    ...
    while(1)
    {
        fflush(stdin);     //keyboard ch input flush
        strcpy(str,"");
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(Command[0],"write") == 0)
            {
                ...
                if(ret == -2)
                        printf("Error : Theere is no sufficient memory to write\n");
                if(ret == -3)
